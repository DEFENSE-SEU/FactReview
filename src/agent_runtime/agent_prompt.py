...
def _build_review_annotator_prompt(
    *,
    ...
    return (
        ...
        "  (a) Extract all author-stated claims; apply all rules below before finalising the list.\n"
        "  (b) PRIMARY SOURCE: if the paper has an explicit contributions block (a bulleted/numbered list under phrases such as "
        "'\"Our contributions are:\", \"In summary, we:\", \"The main contributions of this paper are:\"'), "
        "use it as the primary extraction source and supplement only with claims from the Abstract not already covered. "
        "Do not extract claims that appear solely in experiment result sections or discussion sections.\n"
        "  (c) LINGUISTIC SIGNAL: each extracted claim must correspond to an active author-assertion — "
        "for Methodological/Data claims use language such as \"We propose / introduce / construct / design / develop / build\"; "
        "for Experimental/Efficiency claims use language such as \"Our method/model achieves / outperforms / demonstrates / is X times faster\". "
        "Exclude passive result sentences (\"Table X shows...\", \"As shown in Figure Y...\") "
        "and ablation observations (\"removing X degrades...\", \"without X the performance drops...\").\n"
        "  (d) CONSOLIDATION: all performance-related claims about the same capability or task — "
        "including different metrics, percentage gains, and sub-task results — must be merged into one unified claim; "
        "sub-metric quantifications are supporting evidence, not separate claims.\n"
        "  (e) ABLATION EXCLUSION: findings from ablation studies are evidence for a methodology claim, not independent claims; do not list them.\n"
        "  (f) Exclude background statements, related-work descriptions, motivation sentences, minor implementation details, "
        "dataset usage (not construction), and routine training procedures.\n"
        "  (g) Deduplicate: after extraction, merge any two claims referring to the same underlying contribution.\n"
        "  (h) Validity gate: each claim must be (1) explicitly author-asserted using the linguistic signals in (c), "
        "(2) conceptually distinct from every other claim in the final list, "
        "and (3) of sufficient scope to affect the paper's validity or novelty assessment.\n"
        ...
        "- First locate the explicit contributions block (if present) and extract claims from it; supplement with Abstract claims not already covered; "
        "apply the full extraction constraints: consolidate all performance sub-claims per capability into one, "
        "exclude ablation findings, exclude passive result sentences, exclude background/related-work statements, "
        "exclude implementation details, and deduplicate.\n"
...
def _build_fact_review_extractor_prompt(
    *,
    ...
    return (
        ...
        "  (c) Dimension selection: choose the 3-8 niche dimensions that produce an objective comparison, including both overlap dimensions and difference dimensions. Do not choose axes merely because only This Work wins them; if every selected axis is exclusively won by This Work, revise the axes to include omitted common capabilities or report that objective novelty cannot be established from the available related work.\n"
        ...
        "- `2. Technical Positioning`: the matrix is descriptive evidence, not a proof of novelty. Do not use it to imply `first`, `unique`, or `novel` unless Section 3 Claims separately includes and audits that author-positioning claim.\n"
        ...
        "- `3. Claims` SOURCE: extract reviewer-salient author claims, not a summary of the paper. A claim belongs here only if an expert reviewer would need to audit it because it affects novelty, validity, contribution, reproducibility, or scope.\n"
        "- `3. Claims`: begin with `Paper scope:` and `Evaluation scope:` lines, then one blank line.\n"
        "- `3. Claims`: keep the original report layout: scope lines followed directly by the detailed claim audit table. Do not add a separate `Core claims` block in the report; the compact core-claim layer is only for teaser-display data.\n"
        "- `3. Claims`: include novelty/first-work/literature-gap positioning, central method or design contributions, contribution-critical artifact/resource claims, main empirical effects and null results, efficiency/cost/ceiling claims, and causal/explanatory claims drawn from results.\n"
        "- `3. Claims`: do not target or force a fixed row count. Include every reviewer-salient author claim in the table and omit low-salience descriptive facts.\n"
        "- `3. Claims`: remove semantic duplicates before writing the table; if the same assertion appears multiple times, prefer contribution-block or abstract wording.\n"
        "- `3. Claims`: do not extract neutral architecture/pipeline mechanics, routine settings, hyperparameters, model names, top-k values, prompt templates, dataset descriptions, or generic related-work summaries unless the manuscript makes them part of a reviewed contribution, result, or positioning claim.\n"
        "- `3. Claims`: do not turn every table cell or sub-metric into a claim. If a number only supports a broader takeaway in the same paragraph/section, cite it as evidence for that broader claim instead of adding a separate row.\n"
        "- `3. Claims`: use your judgment to keep claims atomic and independently checkable. Do not merge assertions that need different evidence, such as method existence vs. novelty, resource count vs. public availability, result description vs. causal explanation, or positioning vs. implementation detail.\n"
        "- `3. Claims`: each row should have one main verifiable predicate. If one sentence says both that a resource has a given size and that it is public, both that a method improves a score and reaches an oracle ceiling, both that agents use retrieved items and performance does not improve, or both retrieval coverage and agent use rate, split it into separate rows.\n"
        "- `3. Claims`: granularity examples — "
        "Too broad: 'We present the first X method over a 36K corpus and improve Pass@1.' "
        "Better: three claims — 'This is the first X method'; 'The corpus contains 36K items'; 'The method improves Pass@1 from A to B.' "
        "Too broad: 'The method improves Pass@1 and reaches 84% of the oracle ceiling.' "
        "Better: one claim for the improvement, one for the oracle-ceiling ratio. "
        "Too broad: 'Agents use retrieved skills but performance does not improve.' "
        "Better: one claim for use rate, one for null performance gain. "
        "Too fine: 'Stage 1 has R@1000=0.905; Stage 2 MRR=0.587; Stage 3 MRR=0.634.' "
        "Better: 'Each stage progressively improves ranking quality' — unless one stage's result is itself the headline contribution.\n"
        "- `3. Claims`: when extracting novelty or positioning, do not keep extra artifact, corpus, benchmark, or implementation clauses inside that novelty claim; extract those other assertions separately only if reviewer-salient.\n"
        "- `3. Claims`: treat author positioning as a claim when the manuscript asserts first-work status, novelty, uniqueness, or innovation. Assess it against Related Work and retrieved external papers instead of folding it into a generic method claim.\n"
        "- `3. Claims`: provide a markdown table with columns `Claim | Type | Importance | Evidence | Assessment | Location`.\n"
        "- `3. Claims`: `Type` must be exactly one of `empirical` / `methodological` / `theoretical`: "
        "`empirical` for numeric results, comparisons, statistical tests, null results, efficiency/cost/latency claims, or benchmark statistics; "
        "`methodological` for proposed methods, frameworks, designs, or resource construction; "
        "`theoretical` for conceptual framing, explanatory/causal interpretation, generality, or literature-positioning claims that are not themselves numeric. "
        "`Importance` must be `primary` for claims that directly drive the paper's novelty or validity assessment (core contribution, headline result, main causal claim), "
        "or `secondary` for supporting claims a reviewer still needs to check but that do not stand alone as a key contribution. "
        "`primary` claims must be stated at headline contribution level — they may synthesize several related secondary claims into one broader assertion that a non-specialist can read "
        "(e.g., 'The proposed method achieves state-of-the-art on knowledge graph completion tasks' rather than three separate metric rows for MRR/Hits@1/Hits@10). "
        "`secondary` claims carry the specific metrics, sub-results, ablation components, and causal interpretations that underpin each primary claim. "
        "Before writing a primary claim, ask: would this read naturally as a top-3 bullet in a one-paragraph paper summary? If not, mark it secondary. "
        "Write all rows in descending importance order: most important primary claim first, least important secondary claim last. "
        "Within primary claims, order by centrality to the paper's key contribution (novelty/positioning assertion first, then main empirical result, then methodological claim). "
        "Within secondary claims, order by how much the claim affects validity assessment (claims whose failure would undermine a primary claim come before peripheral supporting details).\n"
        ...
        "- `3. Claims`: `Assessment` must be evidence-driven and type-aware: for `empirical` type, evaluate numeric/empirical support; for `theoretical` type, evaluate theorem/proof/derivation support; for `methodological` type, evaluate design-description and implementation traceability support. Avoid repeated boilerplate sentences.\n"
