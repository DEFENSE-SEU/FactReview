...
def cconv(a, b):
	"""Circular convolution using torch.fft (PyTorch>=1.8)."""
	A = torch.fft.rfft(a, dim=-1)
	B = torch.fft.rfft(b, dim=-1)
	return torch.fft.irfft(A * B, n=a.shape[-1], dim=-1)

def ccorr(a, b):
	"""Circular correlation using torch.fft (PyTorch>=1.8)."""
	A = torch.fft.rfft(a, dim=-1)
	B = torch.fft.rfft(b, dim=-1)
	return torch.fft.irfft(torch.conj(A) * B, n=a.shape[-1], dim=-1)